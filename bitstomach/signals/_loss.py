...
class Loss(Signal):
    ...
    def detect(perf_data: pd.DataFrame) -> Optional[List[Resource]]:
        ...
        for comparison_signal in negative_comparison_signals:
            ...
            streak_length = Loss._detect(perf_data,comparison_signal.value(SLOWMO.RegardingComparator))
    ...
    def _detect(perf_data: pd.DataFrame, comparator: Resource) -> float:
        ...
        first_negative_index = np.argmax(diff_reversed <= 0)
        count = np.sum(diff_reversed[:first_negative_index] > 0)
