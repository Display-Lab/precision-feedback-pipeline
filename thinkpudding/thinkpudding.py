...
class Thinkpudding:
    ...
    def __init__(self, performer_graph, causal_pathways):
        ...
        logging.debug(" reading spek graph--- %s seconds ---" % (time.time() - start_time)) 
        start_time = time.time()
        self.causal_pathways=causal_pathways
        logging.debug(" reading causal pathways graph--- %s seconds ---" % (time.time() - start_time))
