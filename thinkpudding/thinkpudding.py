...
class Thinkpudding:
    ...
    def __init__(self, performer_graph, causal_pathways):
        ...
        logger.debug(" reading spek graph--- %s seconds ---" % (time.time() - start_time)) 
        start_time = time.time()
        self.causal_pathways=causal_pathways
        logger.debug(" reading causal pathways graph--- %s seconds ---" % (time.time() - start_time))
    
    def process_causalpathways(self):
        start_time = time.time()
        self.caus_out_dict_final=process_causalpathways(self.causal_pathways)
        logger.debug(" processing causal pathways--- %s seconds ---" % (time.time() - start_time))

    def process_performer_graph(self):
        start_time = time.time()
        self.performer_graph_out_dicts=process_performer_graph(self.performer_graph)
        logger.debug(" processing spek_cs--- %s seconds ---" % (time.time() - start_time))
    
    def matching(self):
        start_time = time.time()
        self.merged_list=matching(self.performer_graph_out_dicts,self.caus_out_dict_final)
        logger.debug(" processing matching--- %s seconds ---" % (time.time() - start_time))

    def insert(self):
        start_time = time.time()
        self.performer_graph_tp=insert(self.merged_list,self.performer_graph,self.causal_pathways)
        logger.debug(" inserting acceptable by--- %s seconds ---" % (time.time() - start_time))
